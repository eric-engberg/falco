...
#define THROW(cond, err, ctx)    { if ((cond)) { throw rule_loader::rule_load_exception(falco_load_result::FE_LOAD_ERR_VALIDATE, (err), (ctx)); } }
...
std::string rule_loader::context::rules_snippet(const std::string& rules_content) const
{
	std::string ret;

	if(m_locs.empty() || rules_content.size() == 0)
	{
	...
	for(; from > 0 && from >= rules_content.size(); from--);

	// Add the line that includes the mark and a marker
	// at the column number.
	size_t to = from;

	for(; from > 0 && rules_content.at(from) != '\n'; from--);
	for(; to < rules_content.size()-1 && rules_content.at(to) != '\n'; to++);

	// Don't include the newlines
	if(rules_content.at(from) == '\n')
	{
		from++;
	}
	if(rules_content.at(to) == '\n')
	{
		to--;
	}

	ret = rules_content.substr(from, to-from+1) + "\n";
...
void rule_loader::result::add_error(falco_load_result::error_code ec, const std::string& msg, const context& ctx, const std::string& rules_content)
{
	error err = {ec, msg, ctx, ctx.rules_snippet(rules_content)};
	success = false;

	errors.push_back(err);
}

void rule_loader::result::add_warning(falco_load_result::warning_code wc, const std::string& msg, const context& ctx, const std::string& rules_content)
{
	warning warn = {wc, msg, ctx, ctx.rules_snippet(rules_content)};

	warnings.push_back(warn);
}

const std::string& rule_loader::result::as_summary()
{
	std::ostringstream os;
	...
	if(!errors.empty())
	{
		...
		for(auto &err : errors)
		{
			...
			os << falco_load_result::error_code_str(err.ec)
			   << " (" << falco_load_result::error_str(err.ec) << ")";
		}
		os << "]";
	}
	...
	if(!warnings.empty())
	{
		...
		for(auto &warn : warnings)
		{
			...
			os << falco_load_result::warning_code_str(warn.wc)
			   << " (" << falco_load_result::warning_str(warn.wc) << ")";
		}
		os << "]";
	}
...
const std::string& rule_loader::result::as_string()
{
	std::ostringstream os;
...
const nlohmann::json& rule_loader::result::as_json()
{
	...
	for(auto &err : errors)
	{
		...
		jerr["context"]["snippet"] = err.rules_snippet;

		jerr["code"] = falco_load_result::error_code_str(err.ec);
		jerr["codedesc"] = falco_load_result::error_desc(err.ec);
	...
	for(auto &warn : warnings)
	{
		...
		jwarn["context"]["snippet"] = warn.rules_snippet;

		jwarn["code"] = falco_load_result::warning_code_str(warn.wc);
		jwarn["codedesc"] = falco_load_result::warning_desc(warn.wc);
	...
	const rule_loader::context &ctx)
{
	...
	catch (const sinsp_exception& e)
	{
		throw rule_loader::rule_load_exception(
			falco_load_result::FE_LOAD_ERR_COMPILE_CONDITION,
			e.what(),
			ctx);
	}
}
...
void rule_loader::define(configuration& cfg, rule_info& info)
{
	...
	if (!source)
	{
		cfg.res->add_warning(falco_load_result::FE_LOAD_UNKNOWN_SOURCE,
...
rule_loader::rule_load_exception::rule_load_exception(falco_load_result::error_code ec, std::string msg, const context& ctx)
...
{
	errstr = falco_load_result::error_code_str(ec) + ": "
		+ msg.c_str();
	...
	indexed_vector<falco_rule>& out) const
{
	...
	set<falco_load_result::warning_code> warn_codes;
	...
	for (auto &r : m_rule_infos)
	{
		...
		if(!is_format_valid(*cfg.sources.at(r.source), rule.output, err))
		{
			throw rule_load_exception(
				falco_load_result::FE_LOAD_ERR_COMPILE_OUTPUT,
				err,
				r.output_ctx);
		}
		...
		catch (const falco_exception& e)
		{
			...
			{
				cfg.res->add_warning(
					falco_load_result::FE_LOAD_UNKNOWN_FIELD,
					e.what(),
					r.cond_ctx,
					cfg.content);
			}
		...
		if(rule.source == falco_common::syscall_source)
		{
			...
			{
				cfg.res->add_warning(
					falco_load_result::FE_LOAD_NO_EVTTYPE,
...
void rule_loader::compile(configuration& cfg, indexed_vector<falco_rule>& out) const
{
	...
	for (auto &m : macros)
	{
		...
		{
			cfg.res->add_warning(
				falco_load_result::FE_LOAD_UNUSED_MACRO,
	...
	for (auto &l : lists)
	{
		...
		{
			cfg.res->add_warning(
				falco_load_result::FE_LOAD_UNUSED_LIST,
